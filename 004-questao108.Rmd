...
$$
V_e = (Z_T)(I_s) + V_s = (`r round(sol[["ZEQ"]],2)`)(`r round(sol[["is"]],2)`) + `r round(sol[["vss"]],2)`
$$
...
resultados <- data.frame(T1 = c("$V_e$","$I_e$","$S_T$", "$fp$"),
                         T2 = c(paste(round(Mod(sol$ve),2), "$\\angle ($", round(rad2degree(Arg(sol$vpp)),2), "$)^\\circ V$"),
                                paste(round(Mod(sol$ie),2),"$\\angle ($", round(rad2degree(Arg(sol$ip)),2), "$)^\\circ A$"),
